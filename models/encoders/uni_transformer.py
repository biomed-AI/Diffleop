...
class UniTransformerO2TwoUpdateGeneral(nn.Module):
    ...
    def forward(self, h, x, mask_ligand, batch, return_all=False, fix_x=False, mask_ligand_atom = None, mask_mask = None,):
        ...
        for b_idx in range(self.num_blocks):
            ...
            if mask_mask is not None:
                mask_mask_idx = torch.nonzero(mask_ligand_atom).squeeze()
                mask_ligand_atom = mask_ligand_atom.clone()
                tmp = mask_ligand_atom[mask_mask_idx]
                tmp[~mask_mask] = False
                mask_ligand_atom[mask_mask_idx] = tmp

            for l_idx, layer in enumerate(self.base_block):
                h, x = layer(h, x, edge_type, edge_index, mask_ligand_atom, e_w=e_w, fix_x=fix_x)
            all_x.append(x)
            all_h.append(h)
