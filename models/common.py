...
def compose_context_with_prior(
        h_protein, h_ligand, h_prior,
        pos_protein, pos_ligand, pos_prior,
        batch_protein, batch_ligand, batch_prior,
        protein_group_idx, ligand_group_idx, prior_group_idx, ligand_atom_mask):

    batch_ctx = torch.cat([batch_protein, batch_ligand, batch_prior], dim=0)
    sort_idx = torch.sort(batch_ctx, stable=True).indices

    mask_ligand = torch.cat([
        torch.zeros([batch_protein.size(0)], device=batch_protein.device).bool(),
        torch.ones([batch_ligand.size(0) + batch_prior.size(0)], device=batch_ligand.device).bool(),
        # torch.ones([batch_ligand.size(0)], device=batch_ligand.device).bool(),
        # torch.zeros([batch_prior.size(0)], device=batch_ligand.device).bool(),
    ], dim=0)[sort_idx]

    if ligand_atom_mask is None:
        ligand_atom_mask = torch.ones([batch_ligand.size(0)], device=batch_protein.device).bool()
    mask_ligand_atom = torch.cat([
        torch.zeros([batch_protein.size(0)], device=batch_protein.device).bool(),
        ligand_atom_mask,
        torch.zeros([batch_prior.size(0)], device=batch_protein.device).bool(),
    ], dim=0)[sort_idx]

    batch_ctx = batch_ctx[sort_idx]
    h_ctx = torch.cat([h_protein, h_ligand, h_prior], dim=0)[sort_idx]  # (N_protein+N_ligand, H)
    pos_ctx = torch.cat([pos_protein, pos_ligand, pos_prior], dim=0)[sort_idx]  # (N_protein+N_ligand, 3)
    group_idx_ctx = torch.cat([protein_group_idx, ligand_group_idx, prior_group_idx], dim=0)[sort_idx]
    protein_index_in_ctx, ligand_index_in_ctx = find_index_after_sorting(
        len(h_ctx), len(h_protein), len(h_ligand), sort_idx, batch_protein.device)
    return h_ctx, pos_ctx, group_idx_ctx, batch_ctx, mask_ligand, mask_ligand_atom, protein_index_in_ctx, ligand_index_in_ctx
