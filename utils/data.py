...
def get_pocket(mol, pdb_path):
    ...
    AA_NAME_NUMBER = {
        k: i for i, (k, v) in enumerate(AA_NAME_SYM.items())
    }
    ...
    for residue in struct.get_residues():
        ...
        for atom in residue.get_atoms():
            ...
            atomic_number = ptable.GetAtomicNumber(atom_type)
            element.append(atomic_number)
            pos.append(atom_coord.tolist())
            is_backbone.append(atom_n in {'N', 'CA', 'C', 'O'})
            atom_name.append(atom_n)
            atom_to_aa_type.append(AA_NAME_NUMBER[resname])
...
def parse_sdf_file(path, kekulize=True):
    ...
    data['smiles'] = Chem.MolToSmiles(rdmol)
    return data
