...
from datasets.pl_pair_dataset_affinity import get_dataset_linker_aff, get_dataset_dec_aff, get_dataset_aff_for_case
...
if __name__ == '__main__':
    ...
    parser.add_argument('--protein_filename', type=str, default='/path/to/example.pdb')
    parser.add_argument('--ligand_filename', type=str, default='/path/to/example.sdf')
    parser.add_argument('--anchor_id_given_1', type=int, default=0)
    parser.add_argument('--anchor_id_given_2', type=int, default=0)
    ...
    if args.type == 'linker':
        train, test = get_dataset_aff_for_case(
            config=config.data,
            protein_filename=args.protein_filename,
            ligand_filename=args.ligand_filename,
            anchor_id_given_1=args.anchor_id_given_1,
            anchor_id_given_2=args.anchor_id_given_2,
            transform=transform,
        )
    elif args.type == 'dec':
        train, test = get_dataset_aff_for_case(
    ...
    for batch_idx, batch in enumerate(test_loader):
        ...
        for i in tqdm(range(starting_point, config.sample.num_samples), desc=str(batch_idx)):
            print('[DEBUG] enter sample diffusion')
            ...
            print('[DEBUG] out sample diffusion')
